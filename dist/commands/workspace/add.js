 ...
 */
import { addRepository } from "@baseline/core/commands";
import { Logger } from "@baseline/core/utils";
import { ConfigManager } from "@baseline/core/config";
export async function addCommand(gitUrl, options = {}) {
    try {
        ...
        Logger.error(`Failed to add repository: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
