 ...
 */
import { initWorkspace } from "@baseline/core/commands";
...
export async function initCommand(options = {}) {
    ...
    try {
        ...
        Logger.log(`  1. Run \`${commandName} add <gitUrl>\` to add repositories`);
        Logger.log(`  2. Run \`${commandName} clone\` to clone all repositories`);
        Logger.log("  3. Run `baseline status` to check repository status");
    }
    catch (error) {
        Logger.error(`Failed to initialize workspace: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
