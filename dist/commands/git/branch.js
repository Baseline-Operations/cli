 ...
 */
import { branchRepositories } from "@baseline/core/commands";
import { Logger } from "@baseline/core/utils";
export async function branchCommand(branchName, options = {}) {
    try {
        ...
        Logger.error(`Failed to process branches: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
