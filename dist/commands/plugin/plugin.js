 ...
 */
import { installPlugin, listPlugins, removePlugin, installAllPlugins, searchPlugins, } from "@baseline/core/commands";
import { Logger } from "@baseline/core/utils";
export async function pluginInstallCommand(pluginId, options = {}) {
    try {
...
export async function pluginRemoveCommand(pluginId, options = {}) {
    try {
        ...
        Logger.error(`Failed to remove plugin: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
export async function pluginInstallAllCommand(options = {}) {
    try {
        ...
        Logger.error(`Failed to install all plugins: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
export async function pluginSearchCommand(query, options = {}) {
    try {
        ...
        Logger.error(`Failed to search plugins: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
