 ...
 */
import { releaseCommand as releaseCommandCore } from "@baseline/core/commands";
...
export async function releaseCommand(subcommand, options = {}) {
    try {
        ...
        if (subcommand === "plan" && "table" in result && result.table) {
            Logger.table(["Package", "Current Version", "Path", "Changes"], result.table);
        }
        if (!result.success) {
            process.exit(1);
        }
        ...
        Logger.error(`Failed to run release: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
    }
}
